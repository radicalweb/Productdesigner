...
class Reorder extends \Magento\Sales\Model\Reorder\Reorder
{
	...
	public function __construct(
		...
		\Magento\Sales\Model\Reorder\OrderInfoBuyRequestGetter $orderInfoBuyRequestGetter
	) {
		...
		parent::__construct($orderFactory, $customerCartProvider, $guestCartResolver, $cartRepository, $reorderHelper, $logger, $productCollectionFactory, $orderInfoBuyRequestGetter);
	}
	...
	private function addItemToCart(OrderItemInterface $orderItem, Quote $cart, ProductInterface $product): void
	{
		...
		// echo '<pre>';print_r($extraData);exit;
		...
		try {
			...
			$addProductResult->addOption(array(
				'product' => $addProductResult->getProduct(),
				'code'  => 'additional_options',
				'value' => json_encode($orderItem->getProductOptionByCode('additional_options'))
			));
		} catch (\Magento\Framework\Exception\LocalizedException $e) {
